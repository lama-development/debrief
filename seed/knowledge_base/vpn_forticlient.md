...
1. Verificare che l'utente abbia connettivita' Internet (puo' navigare?)
...
5. Se nessun utente si connette: il problema e' lato FortiGate
   - Verificare lo stato del servizio VPN sul FortiGate (se si ha accesso)
   - Contattare {{IT_EXTERNAL}}

### Connessione VPN lenta

1. Far testare la velocita' della connessione domestica dell'utente (speedtest.net)
2. Se la banda e' sufficiente: il problema potrebbe essere sul FortiGate
   - Verificare se il DPI (Deep Packet Inspection) e' attivo sul tunnel VPN (aggiunge latenza)
...
2. Se l'accesso e' stato disattivato per inattivita': riattivarlo dal FortiGate (o chiedere a {{IT_EXTERNAL}})
...
- Revisione trimestrale degli accessi fornitori: disattivare quelli non piu' necessari
